        ...
        <div className="container relative">
          ...
          <div className="grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 grid-cols-1 gap-6 mt-6">
            {servicesData.map((item, index) => {
              ...
              return (
                <div
                  className="group rounded-md shadow dark:shadow-gray-700 relative bg-white dark:bg-slate-900 p-6 overflow-hidden h-fit min-h-385"
                  key={index}
                >
                    ...
                    <ul className="text-slate-400 mt-3">
                      {item.tech.map((tech, techIndex) => {
                        ...
                        return (
                          <li
                            key={techIndex}
                            className="flex items-center mt-2 ml-0-important group-hover:text-dark dark:group-hover:text-white"
                          >
                            {TechIcon && <TechIcon className="mr-2" />}{" "}
                            {tech.name}
                          </li>
                        );
                      })}
                  ...
                  <div className="mt-4">
                    <Link to="/service" className="text-teal-500">
                      Read More{" "}
